...
class Blockchain {
    ...
    requestMessageOwnershipVerification(address) {
        return new Promise((resolve) => {
            ...
            let requestMessageOwnership = `${address}:${new Date().getTime().toString().slice(0,-3)}:starRegistry`;
            resolve(requestMessageOwnership);
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            const getTimeFromMsg = parseInt(message.split(':')[1]);
            const currentTime = parseInt(new Date().getTime().toString().slice(0, -3));

            if (currentTime - getTimeFromMsg > (5 * 60)) reject(new Error('Time elapsed is greater than 5 minutes'));

            const validSign = bitcoinMessage.verify(message, address, signature);
            if (!validSign) reject(new Error('Verification failed'));
